...
-- 생성일시: 2026-06-06 16:23:10
-- 대상 테이블: DS3(CUSTOMERS, ORDERS, PRODUCTS, ORDERLINES 등)
--              Grocery Oracle(jta_* 테이블)
-- 탐지 기반 패턴: P01, P03, P04, P05, P06, P07, P09, P10, P12, P13, P14, P15, P17, P19, P20, P21, P22, P23, P24, P25, P26, P27, P28, P29, P30
-- ============================================================
...
-- 🚨 [P04] NVL Function (MEDIUM): ORDERS netamount NVL null 치환
SELECT orderid, NVL(netamount, 0) FROM ORDERS WHERE customerid = 500;

-- 🚨 [P04] NVL Function (MEDIUM): INVENTORY quan_in_stock NVL
...
-- 🚨 [P06] VARCHAR2 Usage (MEDIUM): CUSTOMERS 임시 테이블 VARCHAR2
...
-- 🚨 [P15] SYSDATE Usage (MEDIUM): ORDERS 최근 30일 SYSDATE 비교
SELECT * FROM ORDERS WHERE orderdate >= SYSDATE - 30;

-- 🚨 [P15] SYSDATE Usage (MEDIUM): CUSTOMERS 등록일 SYSDATE 비교
...
-- 🚨 [P25] NUMBER Type Declaration (MEDIUM): 임시 주문 테이블 NUMBER 타입
